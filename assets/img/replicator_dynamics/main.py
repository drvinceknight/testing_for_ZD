...
strategies_of_interest = ["ZD-GTFT-2",
                          "ZD-GEN-2",
                          "ZD-Extort-2",
                          ...
                          "Tit For Tat",
                          "Win-Stay Lose-Shift"]
strategies_of_interest_indices = np.array([i for i, s in enumerate(strategies)
                                           if s in strategies_of_interest])
...
def main(process_data=False):
    ...
    rank_of_strategies_of_interest = [np.where(sorted_indices == ele)[0][0]
    ...
    for rank, indx in zip(rank_of_strategies_of_interest,
                          strategies_of_interest_indices):
        name = strategies[indx]
        long_run_prob = xs[-1][rank]
        y = np.cumsum(xs[-1])[rank] - xs[-1][rank]/2
        plt.annotate(f"{name}: $x_{{{rank + 1}}}={round(long_run_prob, 3)}$",
                     xy=(4.95, y),
                     xycoords='data',
                     xytext=(5.5, y),
                     textcoords='data',
