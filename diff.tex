...
%DIF DEL old.tex   Sat Mar 16 10:07:27 2024
%DIF ADD new.tex   Sat Mar 16 10:10:42 2024
...
%DIF -------
\author[3]{Nikoleta E. Glynatsi} %DIF > 
%DIF -------
...
%DIF 31a31-32
\affil[3]{Max Planck Research Group on the Dynamics of Social Behavior, Max %DIF > 
Planck Institute for Evolutionary Biology, Pl\"on, Germany} %DIF > 
%DIF -------
\date{\today}
...
thereof\DIFaddbegin \DIFadd{, }\DIFaddend of zero-determinant strategies is done by placing some
...
strategies will be observed and their \DIFdelbegin \DIFdel{extortionateness }\DIFdelend \DIFaddbegin \DIFadd{intensity of extortion }\DIFaddend quantified empirically.
...
\DIFdelbegin \DIFdel{opponents }\DIFdelend \DIFaddbegin \DIFadd{opponent's }\DIFaddend previous play, led to much research concentrating on these so called
...
with evolutionary dynamics that offer some insight \DIFdelbegin \DIFdel{in to }\DIFdelend \DIFaddbegin \DIFadd{into }\DIFaddend the
...
which are defined by elements of \(\mathbb{R}^4\)\DIFaddbegin \DIFadd{, }\DIFaddend mapping a state of
...
In Section~\DIFdelbegin %DIFDELCMD < \label{sec:subspace_of_extortionate_strategies}%%%
\DIFdelend \DIFaddbegin \DIFadd{\ref{sec:subspace_of_extortionate_strategies}}\DIFaddend , the reverse problem is considered: given a
...
\begin{equation}\label{eqn:definition_of_chi}
    \chi = \frac{\tilde p_2 (P - T) + \tilde p_3 (S - P)}{\tilde p_2 (P - S) + \tilde p_3 (T - P)}
\end{equation}
...
%DIFDELCMD <     p_1 & = \frac{(R-P)(p_2 + p_3) - R + T + S - P}{S + T - 2P}
%DIFDELCMD <      \label{eqn:condition_for_p1}\\
%DIFDELCMD <     p_4 & = 0 \label{eqn:condition_for_p4}\\
%DIFDELCMD <     1 & > p_2 + p_3\label{eqn:condition_for_chi}
%DIFDELCMD < \end{align}%%%
...
\DIFdelbegin \DIFdel{All }\DIFdelend \DIFaddbegin \DIFadd{Based on Equations (\ref{eqn:condition_for_p1}-\ref{eqn:condition_for_chi}), 
it is evident that all }\DIFaddend extortionate strategies
reside on a triangular \DIFdelbegin \DIFdel{(\ref{eqn:condition_for_chi})
plane (\ref{eqn:condition_for_p1}) in 3 dimensions (\ref{eqn:condition_for_p4})}\DIFdelend \DIFaddbegin \DIFadd{plane within a three-dimensional space}\DIFaddend .
...
3, 0)\) from~\cite{Stewart2012}\DIFaddbegin \DIFadd{, }\DIFaddend which is referred to as Extort-2. In
...
As an example consider Table~\ref{tab:actual_plays_of_ZDextort-2}\DIFaddbegin \DIFadd{, }\DIFaddend which shows
...
%DIFDELCMD <     -x^*_2 < x^*_1 \label{eqn:measured_condition_for_chi}
...
\DIFdelbegin %DIFDELCMD < \begin{equation}\label{eqn:x_SSError_formula}
%DIFDELCMD <     \SSe = {\tilde{\textbf{p}}} ^ T \tilde{\textbf{p}}
%DIFDELCMD <            \tilde{\textbf{p}} C \left(C ^ T C \right) ^ {-1} C ^ T
%DIFDELCMD <            \tilde{\textbf{p}} = {\tilde{\textbf{p}}} ^ T \tilde{\textbf{p}} - \tilde{\textbf{p}} C \textbf{x}^*
%DIFDELCMD < \end{equation}%%%
\DIFdelend \DIFaddbegin \begin{align}\label{eqn:x_SSError_formula}
...
\begin{figure}[!htbp]
    ...
    \caption{\(\SSe\) and best fitting \(\chi\) for~\cite{Stewart2012},
        ordered \DIFaddbeginFL \DIFaddFL{(in descending order) }\DIFaddendFL both by number of wins and overall score.
        \DIFaddbeginFL \DIFaddFL{A win is when a strategy obtains a higher score than the player it is
        interacting with.
        }\DIFaddendFL The strategies with a positive skew
...
\begin{figure}[!htbp]
    ...
    \caption{\(\SSe\) and \DIFaddbeginFL \DIFaddFL{probability of mutual defection (}\DIFaddendFL \(P(DD)\)\DIFdelbeginFL \DIFdelFL{and state probabilities }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{) 
        }\DIFaddendFL for the strategies for
...
\begin{figure}[!htbp]
    ...
    \caption{\DIFaddbeginFL \DIFaddFL{Skew of }\DIFaddendFL \(\SSe\) for all strategies considered over all opponents.
...
Equation (\ref{eqn:replicator_dynamics}) is solved numerically \DIFdelbegin \DIFdel{through an }\DIFdelend \DIFaddbegin \DIFadd{for an initial
population with a uniform distribution of the strategies. This is done using an
}\DIFaddend integration technique described in~\cite{Petzold1983} until a stationary vector
\(\gamma=s\) is found.
...
\begin{table}[!hbtp]
    ...
    \end{center}
    \caption{General linear model \DIFaddbeginFL \DIFaddFL{predicting the stationary probability as a
    function of the mean, median and variance of the \(\SSe\)}\DIFaddendFL . This shows that strategies with a low mean
...
\DIFaddbegin \DIFadd{The Moran Process is an evolutionary model of evolutionary in a finite
...
}\DIFaddend In~\cite{Moran1707} a large data set of pairwise fixation probabilities in the
Moran process is made available at~\cite{vincent_knight_2017_1040129}\DIFaddbegin \DIFadd{.
}\DIFaddend Figure~\ref{fig:compare-fixation-to-sserror} shows linear models fitted to three
summary measures of \(\SSe\) and the mean (over population size \(N\) and
opponents) value of \(\kappa_1\cdot N\)\DIFdelbegin \DIFdel{where \(\kappa_1\) denotes the
probability of a single individual entrant to a population taking over}\DIFdelend . This
...
\begin{table}[!hbtp]
    ...
    \end{center}
    \caption{General linear model \DIFaddbeginFL \DIFaddFL{predicting the mean fixation probability as a
    function of the mean, median and variance of the \(\SSe\)}\DIFaddendFL . This shows that strategies with a high mean
...
\begin{quote}
\DIFdelbegin \DIFdel{``It is not the most intellectual of the species that survives; it is not the
...
itself.''
    ...
    \DIFadd{\textit{``It is not the most intellectual of the species that survives; it is not the
strongest that survives; but the species that survives is the one that is able
to adapt to and to adjust best to the changing environment in which it finds
itself.''
}
...
Rather\DIFaddbegin \DIFadd{, }\DIFaddend more sophisticated strategies are able to adapt to a variety of opponents
