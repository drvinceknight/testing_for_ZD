...
\begin{abstract}
    ...
    This work will conceptually demonstrate that it is possible to recognise
    this extortion using a linear algebraic approach. Further to this, a large
    tournament with
    \input{assets/tex/number_of_full_strategies/main.tex}strategies is
    considered with which it will be demonstrated that sophisticated strategies
    exist that do in fact recognise this behaviour and adapt to their opponents.
    Statistical analysis of these strategies will demonstrated that one of the
    biggest factors correlated to evolutionary stability is in fact
    adaptability.

    This suggests that whilst the theory of Zero
...
extensive numerical examples. All of the code and data discussed in
...
extortionate strategy would be: \((2/2, 1/5, 3/8, 0/4)\) which does not fit
...
\(\SSe\) approximately 0) gains a large number of wins.
...
\(\chi\) and \(\SSe\), whilst Figure~\ref{fig:sse_chi_probabilities_in_full}
seems to indicate that if a strategy has a low \(\SSe\) then it is acting
extortionately \(\chi>1\) this is not necessarily the case.
...
have a large variation in \(\SSe\) whilst the ZD strategies have a low score
but high probability of winning. A version of
Figure~\ref{fig:sserror_in_selection} with all the strategies considered is
available in the appendix and gives the same conclusion. This evidences an idea proposed
...
In Figure~\ref{fig:compare-evolutionary-dynamics-to-sserror} linear models are
fitted to the mean and variance of \(\SSe\) against the long run
probabilities \(s\) of (\ref{eqn:replicator_dynamics}). In both cases the
predictive power of these models is small (a low \(R^2\)) however, we note a
significant positive correlation between the variance of \(\SSe\)
...
their memory one representation depending on the opponent. A general linear
...
\includepdf{assets/img/sserror_in_std/main.pdf}
