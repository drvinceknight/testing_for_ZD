...
\begin{abstract}
    ...
    an extortionate way is described. Furthermore, experimental analysis of a
...
variable \(x\). Letting \(f(x)=\|C x- p^*\|_2^2\), the best fitting \(x^*\) is
defined by:
...
\begin{equation}\label{eqn:r_squared}
    \text{SS}_{\text{error}} = f(x^*)
\end{equation}

Known results~\cite{} can now be applied:  % TODO Include reference text
...
A further known result~\cite{} gives an expression for % TODO Include reference text
\(\text{SS}_{\text{error}}\):

\begin{equation}\label{eqn:x_SSError_formula}
    \text{SS}_{\text{error}} = {p ^ *} ^ T p ^ * - p ^ * C \left(C ^ T C \right) C ^ T p ^ *
\end{equation}
...
Figure~\ref{fig:SSError_overall_in_stewart_plotkin} shows the \(\text{SS}_{\text{error}}\)
...
\begin{figure}[!htbp]
    ...
    \caption{\(\text{SS}_{\text{error}}\) for the strategies for the full tournament. Only
