...
rationally choose to cooperate against an opponent that also has knowledge
...
We can measure how close a strategy is to being extortionate using standard
...
\begin{figure}[!htbp]
    ...
    \caption{\(\SSe\) and best fitting \(\chi\) for~\cite{Stewart2012},
        ...
        extortionate manner. The strategies which a positive skew
...
A version of Figure~\ref{fig:sserror_in_selection}
...
\begin{figure}[!htbp]
    ...
    \caption{\(\SSe\) for the same selection of strategies as
...
\begin{figure}[!hbtp]
    ...
    linear relationship. Strategies with above neutral
    fixation (\(N\cdot x_1>1\)) the \(\SSe\) has negative skew.}
