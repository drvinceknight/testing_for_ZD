...
area is the pair of original computer tournaments run by Robert
...
It is clear that in this case all constraints hold. As a counter example,
...
\begin{figure}[!htbp]
    ...
    \caption{\(\text{SS}_{\text{error}}\) and state probabilities for the strategies
        ...
        states and so no \(\text{SS}_{\text{error}}\) can be computed.
        }
\end{figure}
...
\begin{figure}[!htbp]
    \centering
    \includegraphics[width=.95\textwidth]{./assets/img/SSError_and_probabilities_in_full/main.pdf}
    \caption{\(\text{SS}_{\text{error}}\) for the strategies for the full
        tournament. For the \(\text{SS}_{\text{error}}\) plot only strategy
